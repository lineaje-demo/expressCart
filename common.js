...
const getId = (id) => {
    // Called with no id: mint a fresh ObjectId (preserves getId() behaviour)
    if(id === undefined || id === null){
        return new ObjectId();
    }
    // Only convert to an ObjectId when the value is a valid 24-char hex string.
    // Any other value (empty string, non-hex, wrong length, non-string) is
    // returned untouched so ObjectId() is never called with input that throws.
